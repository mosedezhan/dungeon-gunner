...
```
BootScene → MenuScene → ClassSelectScene → GameScene（并行运行：HUDScene）
...
- **MenuScene** — 标题画面，按 SPACE 或点击启动游戏。
- **ClassSelectScene** — 职业选择界面。展示法师（可选）和战士（占位）两张卡片。选择后通过 `scene.start('GameScene', { class: 'mage' })` 传递职业参数。
- **GameScene** — 主游戏循环。拥有所有物理组（`bullets`、`enemyBullets`、`xpOrbs`、`enemies`、`skillOrbs`）、`Player` 实例和 `WaveManager`。所有碰撞/重叠回调在此处理。通过 `scene.launch('HUDScene')` 并行启动 HUD。`create(data)` 接收 `data.class` 参数初始化玩家职业。
...
- **Player** — 构造函数接受 `classId` 参数（`'mage'` 或 `'player'`），选择对应纹理和动画。持有 `stats` 对象（`PLAYER` 配置的可变副本）。枪是独立的精灵，跟随鼠标旋转。`muzzle` 是每帧更新的 `Vector2`，作为子弹生成点。支持无敌帧和生命回复累积。持有主动技能充能（`skillCharges` / `stats.skillChargesMax`），按 Q 触发 `triggerSkill()`。
...
- **`CLASSES`** — 职业定义（法师/战士），包含纹理 key、技能引用、是否锁定等
