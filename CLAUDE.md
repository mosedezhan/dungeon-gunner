...
所有可调参数的唯一来源：玩家属性、敌人定义、波次时间、XP 曲线、`SKILL` 主动技能参数、`BULLET_TIME` / `TIME_STOP` / `ARCANE_STORM` 技能配置块、升级定义。`UPGRADES` 数组包含 `apply(p)` 函数，直接修改 `Player.stats`；条目可选 `maxLevel` + `levelStat`（同一升级最多抽几次）和 `requires(player)`（解锁条件 gate）。调整游戏平衡时只需修改此文件。
